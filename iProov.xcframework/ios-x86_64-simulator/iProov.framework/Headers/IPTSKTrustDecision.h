 ...
 */
typedef NS_ENUM(NSInteger, IPTSKTrustDecision)
{
    /**
     ...
     This return value does not necessarily mean that the pinning validation succeded (for example if `kIPTSKEnforcePinning` was set to `NO` for this domain). If a pinning validation failure occured and if a report URI was configured, a pin failure report was sent.
     */
    IPTSKTrustDecisionShouldAllowConnection,
     ...
     */
    IPTSKTrustDecisionShouldBlockConnection,
     ...
     */
    IPTSKTrustDecisionDomainNotPinned,
};
